...
async function loadCustomersFromFirestore() {
  try {
    ...
    if (!customers.find((c) => c.id === selectedCustomerId) && customers.length > 0) {
  ...
  const html = `
    <div class="min-h-screen bg-slate-50 py-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-[260px,1fr] gap-4">
        ...
        <!-- 左：カスタマー一覧（今は仮の1件だけ） -->
        <section class="bg-white border rounded-lg shadow-s p-3">
        ...
        <ul id="customerList" class="space-y-1"></ul>
        </section>

        <!-- 右：メモ一覧＋入力フォーム -->
        <section class="bg-white border rounded-lg shadow-s p-3 flex flex-col">
          <div class="mb-3">
          ...
          <ul
            id="noteList"
            class="flex-1 overflow-y-auto space-y-2 mb-3 border-t pt-3 text-s"
          ></ul>
